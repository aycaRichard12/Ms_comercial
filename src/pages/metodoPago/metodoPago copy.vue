    ...
    <q-card class="q-mt-md rounded-borders">
      <q-card-section>
        <q-table
          ...
          bordered
          :filter="searchTerm"
          selected-class="bg-light-blue-2"
          table-header-class="bg-primary text-white"
          title-class="bg-primary text-white q-pa-md"
...
const getLeyendaSINOptions = async () => {
  try {
    ...
    const endpoint = `${URL_APICM}api/listaMetodopagoSIN/metodopago/${token}/${tipo}`
...
const fetchMetodosPago = async () => {
  try {
    ...
    const token = contenidousuario[0]?.factura?.access_token
    const tipo = contenidousuario[0]?.factura?.tipo
    const endpoint = `${URL_APICM}api/listaMetodopagoFactura/${idempresa}/${token}/${tipo}`
    const resultado = await peticionGET(endpoint)
    if (resultado[0] === 'error') {
      console.error(resultado.error)
      $q.notify({
