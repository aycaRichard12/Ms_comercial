  ...
  <q-layout view="lHh lpr lff">
    <q-header class="bg-primary text-white">
      <q-toolbar class="q-pa-sm" style="height: 56px">
        ...
        <q-toolbar-title>
          <q-avatar size="150px">
            <img src="src/assets/IMAGOTIPO-02.svg" />
          </q-avatar>
        </q-toolbar-title>
    ...
    <q-page-container>
      <router-view style="background-color: #eeebe2" />
    </q-page-container>
  </q-layout>
</template>
...
function llevarPrimeraPAgina(submenu) {
  ...
  const tabs = cargarPaginasSubMenu(submenuCode)
  if (tabs.length > 0) {
    const firstTab = tabs[0]
    console.log('llendo a navegar')
...
const selectSubmenu = async (submenu) => {
  console.log(llevarPrimeraPAgina(submenu))
  const submenuCode = submenu.codigo.split('-')[0]
